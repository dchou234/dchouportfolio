...
window.addEventListener("load", function() {
	...
	let holderEl = document.getElementById("p5canvas")
	...
	let mainP5 = new p5(
		...
		function(p) {
			...
			p.draw = () => {
				...
				p.noStroke()
				p.rect(0, 0, 100, 10)
				if (tool.mode === "clearBlack") {
					p.background("black")
					p.text("Canvas cleared Black!", 2, 20)
				}
				if (tool.mode === "clearWhite") {
					p.background("white")
					p.text("Canvas cleared White!", 2, 20)
				}
				p.fill(0)
